...
def setup_github():
    ...
    try:
        subprocess.check_output("util/setup-github",
                                shell=True,
...
def git_add(files: List[str]):
    try:
        subprocess.check_output(f'{_UTIL_PATH}/git-add {" ".join(files)}',
                                shell=True,
                                stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        _raise_runtime_exception(e)
...
def git_checkout(branch, create=False):
    try:
        subprocess.check_output(
            f"BRANCH={branch} CREATE={str(create).lower()} {_UTIL_PATH}/git-checkout",
            shell=True,
            stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        _raise_runtime_exception(e)
...
def git_clone(repo, github_setup=False):
    if github_setup:
        setup_github()
    try:
        subprocess.check_output(f"REPOSITORY={repo} {_UTIL_PATH}/git-clone",
                                shell=True,
                                stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        _raise_runtime_exception(e)
...
def git_commit(title, description=""):
    try:
        subprocess.check_output(
            f'TITLE="{title}" DESCRIPTION="{description}" {_UTIL_PATH}/git-commit',
            shell=True,
            stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        _raise_runtime_exception(e)
...
def git_push(test_run=False):
    try:
        subprocess.check_output(
            f'TEST_RUN="{str(test_run).lower()}" {_UTIL_PATH}/git-push',
            shell=True,
            stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        _raise_runtime_exception(e)

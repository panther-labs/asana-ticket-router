...
class CustomerGroupModifier(AirplaneTask):
    ...
    def create_customer_policy_statement(params: dict) -> comments.CommentedMap:
        ...
        return comments.CommentedMap({"Effect": "Allow", "Action": "sts:AssumeRole", "Resource": role})
    ...
    def add_customer_to_group(self, cfn_yaml: comments.CommentedMap, params: dict) -> None:
        policy_statements = get_group_policy_statements(cfn_yaml, params['group_name'], params['policy_name'])

        if self.find_customer_policy_statement(policy_statements, params["aws_account_id"]):
            raise Exception(
                f"Customer '{params['fairytale_name']}' is already added to the {params['group_name']} group")

        customer_policy_statement = self.create_customer_policy_statement(params)
        policy_statements.append(customer_policy_statement)

    def remove_customer_from_group(self, cfn_yaml: comments.CommentedMap, params: dict) -> None:
        policy_statements = get_group_policy_statements(cfn_yaml, params['group_name'], params['policy_name'])

        customer_policy_statement = self.find_customer_policy_statement(policy_statements, params["aws_account_id"])
    ...
    def main(self, params: dict) -> None:
        params["aws_account_id"] = self.get_aws_account_id(params['fairytale_name'])
        ...
        if operation == "Add":
            self.add_customer_to_group(cfn_yaml, params)
        elif operation == "Remove":
            self.remove_customer_from_group(cfn_yaml, params)
        else:
...
def main(params):
    CustomerGroupModifier().main(params)

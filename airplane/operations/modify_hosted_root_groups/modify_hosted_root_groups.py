...
class HostedRootGroupsModifier(AirplaneTask):
    ...
    def add_user_to_group(self, cfn: comments.CommentedMap, params: dict) -> bool:
        ...
        if self.find_user_in_group(group_members, cfn_export_name):
    ...
    def remove_user_from_group(self, cfn: comments.CommentedMap, params: dict) -> bool:
        group_members = get_group_membership_list(cfn, params["group"])
        cfn_export_name = get_cloudformation_export_name(username=params["aws_username"],
                                                         role_arn=CLOUDFORMATION_READ_ONLY_ROLE_ARN)

        member = self.find_user_in_group(group_members, cfn_export_name)
        if not member:
    ...
    def main(self, params) -> None:
        ...
        if operation == "Add":
            is_group_modified = self.add_user_to_group(cfn_yaml, params)
        elif operation == "Remove":
            is_group_modified = self.remove_user_from_group(cfn_yaml, params)
        else:
...
def main(params):
    HostedRootGroupsModifier().main(params)

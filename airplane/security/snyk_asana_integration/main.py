...
class SnykAsanaIntegration(AirplaneTask):
    ...
    def get_snyk_projects(self, snyk_token):
        ...
        endpoint, headers = self.build_endpoint_headers(f"https://snyk.io/api/v1/org/{SNYK_ORG_ID}/projects",
                                                        f"token {snyk_token}")
        ...
        for project in response.get("projects"):
            if project.get("isMonitored") and project.get("type") != "sast":
                ...
                if (issues.get("low") != 0 or issues.get("medium") != 0 or issues.get("high") != 0
                        or issues.get("critical") != 0):
                    print(name)
                    print(type)
                    print(issues)
                    print("-" * 32)
    ...
    def get_issues_per_project(self, project_id, snyk_token):
        endpoint, headers = self.build_endpoint_headers(
            f"https://snyk.io/api/v1/org/{SNYK_ORG_ID}/project/{project_id}/aggregated-issues", f"token {snyk_token}")
        data = {
...
def main(_):
    return SnykAsanaIntegration().main({})

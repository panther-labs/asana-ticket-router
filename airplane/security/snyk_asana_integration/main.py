...
def get_snyk_projects(snyk_token):
    vulnerable_projects = []

    endpoint, headers = build_endpoint_headers(f"https://snyk.io/api/v1/org/{SNYK_ORG_ID}/projects", f"token {snyk_token}")
...
def get_issues_per_project(project_id, snyk_token):
    endpoint, headers = build_endpoint_headers(f"https://snyk.io/api/v1/org/{SNYK_ORG_ID}/project/{project_id}/aggregated-issues", f"token {snyk_token}")
    data = {
...
def dedup_asana_tasks(issues, asana_pat):
    deduped_issues = []

    for issue in issues:
        if asana_task_doesnt_exist(issue.get("id"), asana_pat):
            deduped_issues.append(issue)
    return deduped_issues


def asana_task_doesnt_exist(issue_id, asana_pat):
    endpoint, headers = build_endpoint_headers(
...
def create_asana_tasks(deduped_issues, asana_pat, project_name, project_type):
    endpoint, headers = build_endpoint_headers("https://app.asana.com/api/1.0/tasks", f"Bearer {asana_pat}")
...
def main(params):
    snyk_token, asana_pat = get_secrets()

    print("[+] Finding vulnerable projects")
    vulnerable_projects = get_snyk_projects(snyk_token)

    for project in vulnerable_projects:
        print("[+] Deduplicating existing issues for " + project.get("name"))
        issues = get_issues_per_project(project.get("id"), snyk_token)
        deduped_issues = dedup_asana_tasks(issues, asana_pat)
        print("[+] Creating issues for project " + project.get("name"))
        create_asana_tasks(deduped_issues, asana_pat, project.get("name"), project.get("type"))

...
class CustomerDeploymentGroupRemover(AirplaneTask):
    ...
    def extract_comments(self, policy_statement: comments.CommentedMap) -> list:
        # Flatten comment list (due to CFN policy statement nestedness)
        comments_flattened = self.flatten_list(policy_statement.ca.items.values())
    ...
    def remove_expired_customers(self, cfn_yaml: comments.CommentedMap) -> list:
        ...
        for policy_statement in policy_statements:
            is_expired = False
            fairytale_name = None
            policy_comments = self.extract_comments(policy_statement)
            for comment in policy_comments:
                if fairytale_name is None:
                    fairytale_name = self.extract_fairytale_name(comment.value)
                if not is_expired:
                    is_expired = self.is_membership_expired(comment.value)
            if is_expired:
    ...
    def main(self, params: dict) -> None:
        ...
        removed_customers = self.remove_expired_customers(cfn_yaml)

...
class DefaultTeamException(Exception):
    """Error handling for default team routing."""


def get_team(
    teams_service: service.TeamService, resource: dict
) -> Tuple[service.EngTeam, List[service.MatchResult]]:
    """GetTeam returns the engTeam for a given resource, or a DefaultTeamException if no team could be matched.
    If callers get a DefaultTeamException, the best solution is to call teams_sevice.default_team() and use that.

    """
    team = precedence_match_team(teams_service, resource)
    if team[0] is None:
        raise DefaultTeamException
    return team
